...
Необходимо реализовать модуль, который работает как фильтр AXI4-Stream.
...
BCD-число представляется десятичным числом:

```systemverilog
value = tens * 4'b1010 + ones;
```
...
```bash
git clone https://gitflic.ru/project/swiniopoty/embedded_fpga_impulse.git
cd embedded_fpga_impulse
```
